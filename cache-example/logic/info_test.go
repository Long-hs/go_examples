...
func TestHandlerWD(t *testing.T) {
	url := "http://localhost:8080/writeDelete?id=1&name=test4"
	client := &http.Client{}
	resp, err := client.Get(url)
	if err != nil {
		t.Fatalf("请求失败: %v", err)
...
func TestHandlerRU(t *testing.T) {
	client := &http.Client{}
	url := "http://localhost:8080/readUpdate?id=1"
	resp, err := client.Get(url)
	if err != nil {
		t.Fatalf("请求失败: %v", err)
		return
